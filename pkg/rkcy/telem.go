...
func NewTelem(ctx context.Context, otelcolEndpoint string) (*Telem, error) {
	var err error
	telem := Telem{
		Ctx:             ctx,
...
func (telem *Telem) Start(ctx context.Context, name string) (context.Context, trace.Span) {
	return Tracer().Start(ctx, name)
}

func (telem *Telem) StartFunc(ctx context.Context) (context.Context, trace.Span) {
	return Tracer().Start(ctx, funcName(1))
}

func (telem *Telem) StartRequest(ctx context.Context) (context.Context, string, trace.Span) {
	traceId := NewTraceId()
...
func (telem *Telem) StartStep(ctx context.Context, rtxn *RtApecsTxn) (context.Context, trace.Span, *rkcypb.ApecsTxn_Step) {
	sc := trace.SpanContextFromContext(ctx)
	if !sc.IsValid() {
...
func (telem *Telem) RecordProduceError(name string, traceId string, topic string, partition int32, err error) {
